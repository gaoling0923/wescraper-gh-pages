...
from wescraper.wespider import WeSpider
...
if __name__ == '__main__':
    ...
    data = []
    if len(sys.argv) > 1 and sys.argv[1] in wescraper.config.types:
        data = WeScraper().crawl_key(sys.argv[1], sys.argv[2:])
        import json

        print(json.dumps(data, ensure_ascii=False).encode('utf8'))
    else:
        print('Search type can only be {}'.format(str(wescraper.config.types)))
